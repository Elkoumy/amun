...
process_model_dir = os.path.join("experiment_figures","process_models")
data_dir = "data"
...
for dataset in datasets:
    ...
    for aggregate_type in aggregate_types:
        ...
        view_model(dfg_freq, os.path.join(process_model_dir , "fig_input_unprotected_" + dataset ))
        ...
        for delta in deltas:
            ...
            for i in range(0,no_of_experiments):
                ...
                view_model(dfg_freq_new,os.path.join(process_model_dir+"fig_input_delta_"+str(delta)+"_"+dataset+"_"+str(aggregate_type)+"_"+str(i)))
        ...
        for emd in emds:
            ...
            view_model(dfg_freq_new, os.path.join(process_model_dir , "fig_input_emd_" + str(emd) + "_" + dataset + "_" + str(aggregate_type)))
...
result_log_delta.to_csv(os.path.join(log_dir,"result_log_delta.csv"),index=False)
...
result_log_alpha.to_csv(os.path.join(log_dir,"result_log_alpha.csv"),index=False)
...
delta_logger_time.to_csv(os.path.join(log_dir,"delta_logger_time.csv"), index=False)

delta_logger_freq=pd.DataFrame(delta_logger_freq, columns=["dataset","aggregate_type","emd","delta"])
delta_logger_freq.to_csv(os.path.join(log_dir,"delta_logger_freq.csv"), index=False)
...
result_log_APE_freq.to_csv(os.path.join(log_dir,"result_log_APE_freq.csv"), index=False)

result_log_APE_time= pd.DataFrame.from_records(result_log_APE_time, columns=["dataset","aggregate_type", "delta", "edge", "APE_time"])
result_log_APE_time.to_csv(os.path.join(log_dir,"result_log_APE_time.csv"), index=False)
#plot the results
