...
def xes_to_DAFSA(data_dir,dataset):
    ...
    if dataset.split('.')[-1]=='xes':
        ...
        xes_exporter.export_log(log, os.path.join(data_dir, dataset + ".xes"))
...
def get_DAFSA_Dictionary(data_dir,dataset):
    ...
    os.system("java -Xmx32g -jar \"%s\" \"%s\" \"%s\"" %(os.path.join(root_dir,'flask-server','amun',"XEStoDAFSA","XEStoDAFSA.jar"),data_dir,dataset+".xes"))
